...
import { HeartPulse, Phone, MapPin, ArrowRight } from 'lucide-react';
import {
  Carousel,
  CarouselContent,
  CarouselItem,
  ...
  if (list.length == 0) {
    ...
    return (
      array.map((value, index) => (
        <div key={index} role="status" className="max-w-sm p-4 borders border-gray-300 rounded shadow animate-pulse md:p-6 dark:border-gray-700">
    ...
    <Carousel>
      <CarouselContent>
        {list.map((doctor, index) => index < 5 && (
          ...
          <CarouselItem className=" sm:basis-1/2  md:basis-1/3 xl:basis-1/5  hover:scale-90   transition-all">
            <div key={index} className='grid   bg-white shadow-2xl h-90  border rounded-lg p-2  hover:bg-blue-200  px-5 border-solid  border-blue-300 hover:scale-110 transition-all ease-in-out hover:shadow-2xl 
      ...
      hover:shadow-blue-500/50'>
              ...
              <Link href={`/details/${doctor.id}`} className='justify-self-center  w-fit'>
                <Button variant="outline" className='my-9 justify-self-center hover:bg-blue-500 bg-blue-100 text-blue-500 hover:text-white  shadow-gray-500 shadow-md font-bold'>

                  more  <ArrowRight className='opaopacity-75' />
                </Button>
              </Link>
